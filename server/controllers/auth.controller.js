 ...
 static async signUp(req, res) {
  ...
  static async resendVerificationEmail(req, res) {
    try {
      ...
      const { error } = await supabase.auth.resend({ type: "signup", email, options: { emailRedirectTo: `${CLIENT_URL}/auth/callback` } })
      if (error) return res.status(400).json({ message: error.message })
  ...
  static async forgotPassword(req, res) {
    try {
      const { email } = req.body
      if (!email) return res.status(400).json({ message: "Email is required" })
      const { error } = await supabase.auth.resetPasswordForEmail(email, { redirectTo: `${CLIENT_URL}/reset-password` })
      if (error) return res.status(400).json({ message: error.message })
  ...
  static async resetPassword(req, res) {
    try {
      ...
      const { error } = await supabase.auth.updateUser({ password: newPassword }, { accessToken: access_token })
      if (error) return res.status(400).json({ message: error.message })
  ...
  static async getCurrentUser(req, res) {
    try {
      ...
      res.status(200).json({ user: { id: user.id, email: user.email, ...userProfile } })
    } catch (err) { console.error(err); res.status(500).json({ message: "Internal server error" }) }
  }
  ...
  static async refreshToken(req, res) {
    try {
      ...
      if (error || !data.session) return res.status(401).json({ message: "Failed to refresh token" })
